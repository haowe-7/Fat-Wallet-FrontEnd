<template>
	<div class="container">
				...
				:class="{nowEditing: curIndex === index && topicEditing, optEditing: curIndex === index}">
					...
					<ul v-else class="options-list" >
						<li v-for="(option, optIndex) in item.options"
							...
							<ul class="opt-ctrl">
								<li class="opt-ctrl-li" v-if="optIndex !== 0"
								@click="moveUp(optIndex, item.options)">上移</li>
								<li class="opt-ctrl-li" v-if="optIndex !== item.options.length - 1"
								@click="moveDown(optIndex, item.options)">下移</li>
								<li class="opt-ctrl-li" v-else @click="addOption(item.options)">添加</li>
								<li class="opt-ctrl-li" @click="delOption(optIndex, item.options)">删除</li>
							</ul>
						</li>
					</ul>
      ...
      <!-- <span @click="iterator = saveBtn(); iterator.next()">保存问卷</span> -->
			<el-button style="width: 20%; margin: 2%; font-size: 24px;"
        @click="iterator = releaseBtn(); iterator.next()">发布问卷</el-button>
	</div>
